...
/** POST /api/agent/fergus/jobs/<id>/resume — resume a held job. No body required. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ job_id: string }> }) {
  ...
  try {
    ...
    const result = await client.resumeJob(id)
    return NextResponse.json({ job: result })
  } catch (e) {
    return NextResponse.json({ error: 'fergus_resume_job_failed', detail: safeErrorDetail(e) }, { status: 502 })
  }
}
