...
/** POST /api/agent/fergus/jobs/<id>/finalise — convert DRAFT → LIVE */
export async function POST(req: NextRequest, { params }: { params: Promise<{ job_id: string }> }) {
  ...
  try {
    ...
    const job = await client.finaliseJob(id)
    return NextResponse.json({ job })
  } catch (e) {
    return NextResponse.json({ error: 'fergus_finalise_failed', detail: safeErrorDetail(e) }, { status: 502 })
  }
}
